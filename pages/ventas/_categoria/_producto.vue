<template>
  <div>
    <LoaderProducto v-if="loading" />
    <div v-show="!loading">
      <div class="modales">
        <ModalVideo v-if="modalYt" :ytLink="producto.ytLink" @close="modalYt = false" />
        <ModalCarousel @close="modalCarousel = false" class="modal" v-if="modalCarousel" :item="producto"
          :currentImgIndex="currentImgIndex" />
      </div>
      <section class="container producto">
        <div class="img-cont">
          <carousel :perPage="1" loop navigationEnabled paginationEnabled paginationColor="rgb(237 223 223)"
            paginationActiveColor="#007BFF" navigationNextLabel="<div class='next'></div>"
            navigationPrevLabel="<div class='prev'></div>">
            <slide :title="`Imágen ${index + 1}`" @slideclick="MostrarCarousel(index)"
              v-for="(img, index) in producto.imgs" :key="index">
              <LoaderIframe v-if="loadingImg" />
              <img @load="LoadedImg()" :src="img" :alt="
                producto.slug
                  .replace(/-/g, ' ')
        ...
        <div class="producto-texto">
          <div class="producto-info">
            <div class="producto-info-flex">
              <router-link :to="'/ventas/' + this.$route.params.categoria"
                :title="`Volver a ` + producto.categoria.nombre" class="btn-back button-icon">
                <i>
          ...
          <div class="cont-iconos">
            ...
            <div class="iconos">
              <a v-if="producto.pdfLink != ''" title="Ver folleto" :href="producto.pdfLink" target="_blank">
                <i class="pdf-icono"></i>
              </a>
              <button class="button-icon" @click="modalYt = true" v-if="producto.ytLink != ''" title="Ver video">
                <i class="yt-icono"></i>
              </button>
            </div>
          </div>
